...
<?php
				...
				while($res=mysqli_fetch_array($results)){
			        ...
			        $list[]=$sr;
			        if($res['lastlogin']!='0000-00-00 00:00:00'){
				        if($ttl!=0){
				        		$sr=['','მოსწავლეები','სახელი','გვარი','ტელეფონის ნომერი','კლასი'];
				        		$list[]=$sr;
							for($j=0; $j<count($tchrs[$res[0]]); $j++){
								$sr=['',"",$tchrs[$res[0]][$j][1],$tchrs[$res[0]][$j][2],$tchrs[$res[0]][$j][3],$tchrs[$res[0]][$j][4]];
								$list[]=$sr;
							}
							$list[] = ['კოდი', 'სახელი', 'გვარი','რაიონი','ქალაქი', 'სკოლა','საგანი','რეგისტრაცია','შემოვიდა','რამდენჯერ შემოვიდა','ბოლო თარიღი','რამდენი კლასი','რომელი კლასები','სულ მოსწავლე','ყველაზე კლასი','ყველაზე რაოდენობა კლასში'];
						}
					}
