	...
	<div class="main">
		<?php
			...
			for($i=1; $i<count($subjects); $i++) {
				...
				echo $x.'</select>&nbsp;&nbsp;&nbsp;&nbsp;';
		?>
		<div id="qrnkselctdiv" style="display: inline-block;">სირთულე: <select class="slctio" id="qrnkselc"  style="width: 150px;"><option value="0">აირჩიეთ</option><option value="1">მარტივი</option><option value="2">საშუალო</option><option value="3">რთული</option></select></div> &nbsp;&nbsp;&nbsp;&nbsp;კლასი: <select class="slctio" id="classelc"  style="width: 150px;"><?php for($i=$stClass; $i<=$enClass; $i++){ if($slctdClass==$i) echo '<option value="'.$i.'" selected>'.$i.'</option>'; else echo '<option value="'.$i.'">'.$i.'</option>';} { if($slctdClass==10) echo '<option value=10 selected>10,11,12</option>'; else echo '<option value=10>10,11,12</option>';} ?> </select>
		...
		<br> <br> რომელში: <select class="slctio" id="locselc"  style="width: 240px;"><?php if($id!=9) {echo '<option value="1"> სკოლაში</option>';} else { if($id==9) echo'<option value="2">ბანკში</option>';}?></select>
